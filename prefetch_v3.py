...
# Updated by: LCAMORO - Migrated to Python3.x plus several byte-type conversions and output format modifications (SIEM compatible) - 2020/Nov/20
#
# Contact: <lcamoro@pm.me>
...
class Prefetch(object):
    ...
    def convertFileReference(self, buf):
        byteArray = ['%02x' % x for x in buf]
                    
        byteString = b""
        for i in byteArray[::-1]:
            byteString += str.encode(i)
        
        return int(byteString, 16)


    def prettyPrint(self):
        # Prints important Prefetch data in a structured JSON format
        tStamps = []
        vols = []
        rsrc = []
        
        jsonData = {}
        jsonData['pfFileName'] = self.pFileName
        jsonData['executableName'] = str(self.executableName, 'utf-8')
        jsonData['runCount'] = self.runCount
        
        if len(self.timestamps) > 1:
            for ts in self.timestamps:
                tStamps.append(ts)
                
            jsonData['lastExecuted'] = tStamps
        else:
            jsonData['lastExecuted'] = self.timestamps[0]
        
        for v in self.volumesInformationArray:
            jsonData['volumeName'] = str(v["Volume Name"], 'utf-8')
            jsonData['creationDate'] = v["Creation Date"]
            jsonData['serialNumber'] = v["Serial Number"]

        for volume in self.directoryStringsArray:
            for vol in volume:
                vols.append(str(vol, 'utf-8'))

        jsonData['dirStrings'] = vols

        for res in self.resources:
            if res:
                rsrc.append(str(res, 'utf-8'))

        jsonData['resourcesLoaded'] = rsrc
